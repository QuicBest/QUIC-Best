...
import (
	...
	"errors"
	"fmt"
	"github.com/mgranderath/dnsperf/util"
...
func (c *baseClient) getTLSDialContext(collector *metrics.Collector, scanType string) dialHandler {
	return func(context context.Context, network string, addr string) (net.Conn, error) {
		...
		err = conn.Handshake()
		collector.TLSHandshakeFinished(TLSVerDict[conn.ConnectionState().Version])
		if err != nil {
			conn.Close()
		...
		if scanType == "verify" {
			...
			certvalid, certerror := checkcert.CheckCertsChain(conn.ConnectionState().PeerCertificates)
			collector.CertValid(certvalid)
			collector.CertError(certerror)

			peercert := conn.ConnectionState().PeerCertificates[0]
			collector.CertLevel(checkcert.GetCertLevel(conn.ConnectionState().PeerCertificates))
